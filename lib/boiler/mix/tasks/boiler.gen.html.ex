defmodule Mix.Tasks.Boiler.Gen.Html do
  @shortdoc "Generates controller, views, and context for an HTML resource"

  @moduledoc """
  Generates controller, views, and context for an HTML resource.

      mix boiler.gen.html Accounts User users name:string age:integer
  ...
  Alternatively, the `--context-app` option may be supplied to the generator:
      ...
      mix boiler.gen.html Sales User users --context-app warehouse
  ...
  module name, for example:
      ...
      mix boiler.gen.html Sales User users --web Sales
  ...
  a `--table` option may be provided. For example:
      ...
      mix boiler.gen.html Accounts User users --table cms_users
  ...
  Read the documentation for `boiler.gen.schema` for more information on
  attributes.
  """
  use Mix.Task
  ...
  def run(args) do
    if Mix.Project.umbrella? do
      Mix.raise "mix boiler.gen.html can only be run inside an application directory"
    end
  ...
  def copy_new_files(%Context{} = context, paths, binding) do
    ...
    Mix.Phoenix.copy_from(paths, "priv/templates/boiler.gen.html", binding, files)
  ...
  defp inputs(%Schema{} = schema) do
    ...
    Enum.map(schema.attrs, fn
      ...
      {_, {:references, _}} ->
        {nil, nil, nil}
      {key = :image, :string} ->
        {label(key), ~s(<div><%= file_input f, #{inspect(key)} %></div>), error(key)}
      {key, :integer} ->
        {label(key), ~s(<%= number_input f, #{inspect(key)}, class: "uk-input" %>), error(key)}
      {key, :float} ->
        {label(key), ~s(<%= number_input f, #{inspect(key)}, step: "any", class: "uk-input" %>), error(key)}
      {key, :decimal} ->
        {label(key), ~s(<%= number_input f, #{inspect(key)}, step: "any", class: "uk-input" %>), error(key)}
      {key, :boolean} ->
        {label(key), ~s(<%= checkbox f, #{inspect(key)}, class: "uk-checkbox" %>), error(key)}
      {key, :text} ->
        {label(key), ~s(<%= textarea f, #{inspect(key)}, class: "uk-textarea" %>), error(key)}
      {key, :date} ->
        {label(key), ~s(<%= date_select f, #{inspect(key)}, class: "uk-input" %>), error(key)}
      {key, :time} ->
        {label(key), ~s(<%= time_select f, #{inspect(key)}, class: "uk-input" %>), error(key)}
      {key, :utc_datetime} ->
        {label(key), ~s(<%= datetime_select f, #{inspect(key)}, class: "uk-input" %>), error(key)}
      {key, :naive_datetime} ->
        {label(key), ~s(<%= datetime_select f, #{inspect(key)}, class: "uk-input" %>), error(key)}
      {key, _}  ->
        {label(key), ~s(<%= text_input f, #{inspect(key)}, class: "uk-input" %>), error(key)}
    end)
  end

  defp label(key) do
    ~s(<%= label f, #{inspect(key)}, class: "uk-form-label" %>)
  end
