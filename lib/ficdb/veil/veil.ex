defmodule Ficdb.Veil do
  @moduledoc """
  Veil's main context
  """
  alias Ficdb.Repo
  alias Ficdb.Directory.{Bookshelf}
  alias Ficdb.Veil.{User, Request, Session}
  alias FicdbWeb.Veil.{Mailer, LoginEmail}
  alias Veil.Secure
  ...
  def get_all_users() do
    User |> Ficdb.SchemaHelpers.order_by_query(:username) |> Repo.all()
  end
  ...
  def create_user(email, username) do
    ...
    |> Ecto.Changeset.put_assoc(:bookshelves, Bookshelf.base_bookshelves |> Enum.map(&%{name: &1}))
    |> Repo.insert()
  end
  ...
  defp new_session_url(conn, unique_id) do
    ...
    cur_path = FicdbWeb.Router.Helpers.session_path(conn, :create, unique_id)
    
    FicdbWeb.Router.Helpers.url(cur_uri) <> cur_path
  end
  ...
  def verify(%Session{} = session) do
    verify(FicdbWeb.Endpoint, session)
  end

  def verify(%Request{} = request) do
    verify(FicdbWeb.Endpoint, request)
  end
